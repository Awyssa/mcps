...
export default function apiBasedTools(server) {
	...
	server.registerTool(
		...
		async (params) => {
			...
			const result = await makeRequest("POST", `${API_BASE_URL}/tags`, tagData, { headers: { "x-api-key": apiKey } });
	...
	server.registerTool(
		...
		async ({ apiKey }) => {
			const result = await makeRequest("POST", `${API_BASE_URL}/auth/api-key/verify`, { key: apiKey });
